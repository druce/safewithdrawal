...
ggplot(data=plotframe, aes(x=worst, y=ce1, colour=frontier, shape=frontier)) +
    scale_x_continuous() +
    ylab("Lifetime CE spending, gamma=1") +
    xlab("Shortfall %") +
    theme_bw() +
    ...
    labs(title="Lifetime CE spending (gamma=1) vs. shortfall, selected percentiles", size=16) +
...
ggplot(data=plotframe0, aes(x=worst, y=ce1, colour=frontier, shape=frontier)) +
    scale_x_continuous() +
    ylab("Lifetime CE spending, gamma=1") +
    xlab("Shortfall %") +
    theme_bw() +
    ...
    labs(title="Lifetime CE spending (gamma=1) vs. worst shortfall", size=16) +
    opts(legend.text=theme_text(size=12), axis.title.x=theme_text(size=12), axis.title.y=theme_text(size=12)) +
    #    geom_point(size=3) +
    geom_line(size=1) +
